...
func toKey(c: int): KeyboardKey =
  result = KeyboardKey(key: c)
  try:
    result.kind = Key(c)
...
when defined(windows):
  ...
  proc eventToString*(ev: KEY_EVENT_RECORD): string =
    ## Конвертирует KEY_EVENT_RECORD → строку (обычно один юникод-символ).
    # Только по нажатиям
    if ev.eventType != 1 or not ev.bKeyDown:
      return ""

    # 1) Если в uChar уже лежит символ — вернём его сразу
    if ev.uChar != 0:
      return $(cast[Rune](ev.uChar))

    # 2) Иначе — раскодируем виртуальную клавишу + состояние
    var keyState: array[256, BYTE]
    if not GetKeyboardState(addr keyState):
      return ""

    var buf: array[4, wchar_t]
    let cnt = ToUnicodeEx(
      ev.wVirtualKeyCode,
      ev.wVirtualScanCode,
      addr keyState,
    ...
    if cnt > 0:
      # buf[0]…buf[cnt-1] — UTF-16 код единичного символа
      # приводим первый wchar к Rune и сразу в string
      return $(cast[Rune](buf[0]))
    else:
      return ""

  proc getKeyAsync(ms: int): KeyboardKey =
    let event = getchTimeout(int32(ms))

    if event.eventType == -1:
      return KeyboardKey(kind: Key.None)

    echo event
    if event.uChar != 0:
      result = toKey((event.uChar))
    else:
      result = KeyboardKey(key: event.wVirtualScanCode)
      case event.wVirtualScanCode
      of  8: result.kind = Key.Backspace
      of  9: result.kind = Key.Tab
      of 13: result.kind = Key.Enter
      of 32: result.kind = Key.Space
      of 59: result.kind = Key.F1
      of 60: result.kind = Key.F2
      of 61: result.kind = Key.F3
      of 62: result.kind = Key.F4
      of 63: result.kind = Key.F5
      of 64: result.kind = Key.F6
      of 65: result.kind = Key.F7
      of 66: result.kind = Key.F8
      of 67: result.kind = Key.F9
      of 68: result.kind = Key.F10
      of 71: result.kind = Key.Home
      of 72: result.kind = Key.Up
      of 73: result.kind = Key.PageUp
      of 75: result.kind = Key.Left
      of 77: result.kind = Key.Right
      of 79: result.kind = Key.End
      of 80: result.kind = Key.Down
      of 81: result.kind = Key.PageDown
      of 82: result.kind = Key.Insert
      of 83: result.kind = Key.Delete
      of 87: result.kind = Key.F11
      of 88: result.kind = Key.F12
      else:  result.kind = Key.None
  ...
  proc parseStdin[T](input: T): KeyboardKey =
    var ch1, ch2, ch3, ch4, ch5: char
    result = KeyboardKey(kind: Key.None)
      ...
      of '\e':
        if read(input, ch2.addr, 1) > 0:
          if ch2 == 'O' and read(input, ch3.addr, 1) > 0:
            if ch3 in "ABCDFH":
              result.kind = KEYS_D[int(ch3) - int('A')]
            elif ch3 in "PQRS":
              result.kind = KEYS_F[int(ch3) - int('P')]
...
proc getKey*(): KeyboardKey =
  ...
  when defined(windows):
    if result.kind == Key.None:
      if hasMouseInput():
        return KeyboardKey(kind: Key.Mouse)

proc getKeyWithTimeout*(ms = 1000): KeyboardKey =
